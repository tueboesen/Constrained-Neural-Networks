...
def main_protein(c):
    ...
    while epoch < c['epochs']:
        ...
        t1 = time.time()
        aloss_t, aloss_rel_t = use_proteinmodel(model, dataloader_train, train=True, max_samples=1e6, optimizer=optimizer, batch_size=c['batch_size'])
        t2 = time.time()
        if c['use_val']:
            aloss_v, aloss_rel_v = use_proteinmodel(model, dataloader_val, train=False, max_samples=1000, optimizer=optimizer, batch_size=c['batch_size'])
        else:
