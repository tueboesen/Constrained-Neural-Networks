...
from src.utils import fix_seed, convert_snapshots_to_future_state_dataset, DatasetFutureState, run_network, run_network_eq
...
if __name__ == '__main__':
    ...
    args.network = {
        ...
        'irreps_edge_attr': o3.Irreps("{:}x0e+{:}x1o".format(args.n_input_samples,args.n_input_samples)),
        'layers': 6,
        'max_radius': 5,
        'number_of_basis': 8,
    ...
    n_vec_latent = 100
    nlayers = 6
    model  = network_eq_simple(n_vec_in, n_vec_latent, nlayers)
    model.to(device)
    ...
    for epoch in range(c['epochs']):
        t1 = time.time()
        aloss_t, aloss_ref_t, MAE_t, t_dataload_t, t_prepare_t, t_model_t, t_backprop_t = run_network_eq(model, dataloader_train, train=True, max_samples=1e6, optimizer=optimizer, batch_size=c['batch_size'])
        t2 = time.time()
        if c['use_validation']:
            aloss_v, aloss_ref_v, MAE_v, t_dataload_v, t_prepare_v, t_model_v, t_backprop_v = run_network_eq(model, dataloader_val, train=False, max_samples=50, optimizer=optimizer, batch_size=c['batch_size'])
        else:
