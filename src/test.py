...
if __name__ == '__main__':
    ...
    legends = ['-','all_chain','all_triangle','all_chaintriangle','end_chain','end_triangle','end_chaintriangle','reg_chain','reg_triangle','reg_chaintriangle']
    for ii,legend in enumerate(legends):
        ax.semilogy(x, res_mean[ii], '-',label=legend)
        # ax.plot(x, res_mean[ii], '-')
        # ax.fill_between(x, res_mean[ii] - res_std[ii], res_mean[ii] + res_std[ii], alpha=0.2)
    ax.legend()
    plt.ylim(1e-4,1)
